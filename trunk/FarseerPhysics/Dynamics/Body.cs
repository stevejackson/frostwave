...
namespace FarseerGames.FarseerPhysics.Dynamics {
    ...
    public class Body : IIsDisposable  {
        ...
        public OnChangeHandler OnUpdate; 
        public event EventHandler<EventArgs> OnDisposed;

        public delegate void OnChangeHandler(ref Vector2 position, ref float rotation);
        ...
        public Vector2 Position {
            ...
            set { 
                position = value;
                if (OnUpdate != null) {
                    OnUpdate(ref position, ref rotation);
                }
            }
        }
        ...
        public void GetVelocityAtWorldPoint(Vector2 worldPoint, Vector2 velocity)
        {
        ...
        private void ApplyDrag() {
            ...
            if (speed > linearDragVelocityThreshhold) {
                #region INLINE: Vector2.Multiply(ref linearVelocity, ref linearVelocity, out linearDrag);
                linearDrag.X = linearVelocity.X * linearVelocity.X;
                linearDrag.Y = linearVelocity.Y * linearVelocity.Y;
                #endregion

                #region INLINE:Vector2.Multiply(ref linearDrag, linearDragCoefficient, out linearDrag);
                linearDrag.X = linearDrag.X * linearDragCoefficient;
                linearDrag.Y = linearDrag.Y * linearDragCoefficient;
                #endregion

                linearDrag.X = -Math.Sign(linearVelocity.X) * linearDrag.X;
                linearDrag.Y = -Math.Sign(linearVelocity.Y) * linearDrag.Y;
