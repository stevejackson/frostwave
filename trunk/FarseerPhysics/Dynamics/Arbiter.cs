...
namespace FarseerGames.FarseerPhysics.Dynamics {
    ...
    public class Arbiter : IEquatable<Arbiter> {
        ...
        private void Collide(Geom geometry1, Geom geometry2, ContactList contactList) {
            ...
            if (geometry1.CollisionHandler != null) {
                if (contactList.Count > 0) {
                    if (!geometry1.CollisionHandler(geometry1, geometry2, contactList)) {
                        contactList.Clear();
                    }
                }
            }
            ...
            //allow user to cancel collision if desired
            if (geometry2.CollisionHandler != null) {
                if (contactList.Count > 0) {
                    if (!geometry2.CollisionHandler(geometry2, geometry1, contactList)) {
                        contactList.Clear();
                    }
                }
            }
